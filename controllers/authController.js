...
const signup = async (req, res) => {
  try {
    ...
    if (!serviceRows.length) {
      return res.status(400).json({ success: false, message: 'Sorry, we do not deliver to pincode ' + pincode + ' yet.' });
    }
    ...
    const [result] = await db.query(
      ...
      [name, email, hashed, phone || null, address || null, pincode, 'customer']
    );

    const user = { id: result.insertId, name, email, role: 'customer', pincode };
...
const updateProfile = async (req, res) => {
  try {
    ...
    if (pincode) {
      ...
      if (!serviceRows.length) {
        return res.status(400).json({ success: false, message: 'We do not deliver to pincode ' + pincode + ' yet.' });
      }
    }

    await db.query(
      'UPDATE users SET name = ?, phone = ?, address = ?, pincode = ? WHERE id = ?',
      [name, phone, address, pincode || null, req.user.id]
    );
